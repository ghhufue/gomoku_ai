...
RewardResult RewardEvaluator::evaluate(
    ...
    } else if (fourThreatTotal(before_opp) > fourThreatTotal(after_opp)) {
        result.block_live_four_bonus = config.block_live_four_bonus;
        result.reward += result.block_live_four_bonus;
        ...
        auto worker = [&](std::size_t start, std::size_t end) {
            for (std::size_t index = start; index < end; ++index) {
                ...
                if (winning_total_local(before_opp) > 0 && winning_total_local(after_opp) == 0) {
                    ...
                    candidate.reward += config.block_live_four_bonus;
